package com.sai.bicheech.controller;

import com.sai.bicheech.model.*;
...
import com.sai.bicheech.service.JwtUserDetailsService;

import com.sai.bicheech.config.JwtTokenUtil;
...
public class JwtAuthenticationController {
	...
	public ResponseEntity<?> saveTeacher(@Valid @RequestBody final UserDTO user, final BindingResult result, final Model model) throws Exception {
		...
		if (!user.getUsername().isEmpty() && userDetailsService.isUsernameExists(user.getUsername())) {
			objErros = new Errors();
			objErros.setField("username");
			objErros.setMessage("This username already exists. Please try a different username.");
			listErrors.add(objErros);
		}
		if (!user.getEmail().isEmpty() && !userDetailsService.studentByEmailExists(user.getEmail()).equals(null)) {
			objErros = new Errors();
